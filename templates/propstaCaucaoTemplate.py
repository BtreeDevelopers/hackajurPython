...
def return_template_caucao(documento, dadosPagamento, pessoa, valor_caucao_formatado, valor_caucao_extenso, is_pf):
    ...
    1.2 - O valor aqui pactuado decorre de dívida oriunda de prestação de serviço telefônico, referente ao contrato x , y e z a qual não fora (m) quitada (s) no vencimento pelo DEVEDOR.
    ...
    2.1.1 – {dadosPagamento.valor_formatado_descontado} ({dadosPagamento.valor_descontado_por_extenso}), inserido {dadosPagamento.valor_desconto}% de desconto na divida ,  a serem pagos em {dadosPagamento.qtd_de_parcela} parcelas no valor de {dadosPagamento.valor_parcela_formatado}, a iniciar no primeiro dia útil após a assinatura do presente instrumento de confissão de dívida com vencimentos no mesmo dia nos meses subsequentes.
    
    Parágrafo Primeiro: O não pagamento de qualquer parcela na data aprazada, importará no vencimento integral e antecipado do débito, sujeitando o DEVEDOR e o AVALISTA, além da execução do presente instrumento, ao pagamento do valor integral do débito, sobre o qual incidirá a aplicação de multa de 10%, juros de mora de 1% ao mês e correção monetária e mais custas processuais e honorários advocatícios na base de 20% sobre o valor total do débito. Os valores monetários serão corrigidos de acordo com a tabela da Corregedoria do Egrégio Tribunal de Justiça de Minas Gerais.
    
    Parágrafo Segundo: Fica autorizado desde já, em caso de inadimplência, o desconto dos valores devidos diretamente no salário da <b>DEVEDOR</b>, até o importe de 30%, bem como penhora de suas contas poupanças, até o limite do débito, ficando estabelecido para tanto negócio jurídico processual, nos termos do art. 190 do Código de Processo Civil (CPC/2015).
    
    <b>CLÁUSULA TERCEIRA</b> – DA CAUÇÃO
    
    O DEVEDOR concorda, desde já, em depositar, a título de caução, no valor de {valor_caucao_formatado} ({valor_caucao_extenso}), valor a ser dado como caução, equivalente a 30% da dívida paga através do cartão de crédito/débito.
    
    Parágrafo Único. O valor da caução será usado em todas as hipóteses em que se façam necessários. Sendo aplicado, tal valor utilizado será imediatamente reposto. Finda  A NEGOCIAÇÃO com a concretização do pagamento INTEGRAL da dívida , o DEVEDOR <b>realizará a solicitação do estorno</b>, sem prejuízo.
    
    <b>CLÁUSULA QUARTA</b> - DA EXIGIBILIDADE DA DÍVIDA
    ...
    <b>CLÁUSULA QUINTA</b> - DO TÍTULO PROTESTADO
    
    4.1- Caso haja título protestado, compete EXCLUSIVAMENTE à DEVEDORA solicitar à CREDORA o termo de anuência para a retirada do protesto.
    ...
    <b>CLÁUSULA SEXTA</b> – DA ADESÃO AO JUIZO 100% DIGITAL
    ...
    <b>CLÁUSULA SÉTIMA</b> –   DA VALIDADE DA ASSINATURA 
